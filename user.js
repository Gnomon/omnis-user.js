//first for generic js functions
...
//tests whether an object on a form has a method
...
omnis_form.prototype.$registerInNotation=function (pForm=$cform){
  ...
  if (MyType=='rf') {
    ...
    if (NotASubwindow) {
      ...
      //register for public use - allow only one instance of this form to be public for now
      //- else need to add the name as unique identifier
    $root.$clientdbs = typeof $root.$clientdbs !== 'undefined' ? $root.$clientdbs : {};
    $root.$clientdbs[MyName] = pForm;
  }
